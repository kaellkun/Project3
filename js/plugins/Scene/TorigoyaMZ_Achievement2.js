/*---------------------------------------------------------------------------*
 * TorigoyaMZ_Achievement2.js v.1.8.2-modified
 *---------------------------------------------------------------------------*
 * Build Date: 2025/09/15 01:14:44 (JST)
 * Modified: 秘密実績の変数による解除機能を追加
 *---------------------------------------------------------------------------*
 * Ruたん ( @ru_shalm )
 ...
 * @default false
 *
 * @param revealVariableId
 * @text 秘密解除変数ID
 * @desc 秘密実績の解除判定に使用する変数のID
 * 0の場合は変数による解除を行いません
 * @type variable
 * @default 0
 *
 * @param revealThreshold
 * @text 秘密解除閾値
 * @desc 指定した変数がこの値を超えたら秘密が解除されます
 * （例: 10を設定すると、変数が11以上で解除）
 * @type number
 * @min 0
 * @decimals 0
 * @default 0
 ...
 * @text 秘密解除スイッチID
 * @desc 秘密実績の解除判定に使用するスイッチのID
 * 0の場合はスイッチによる解除を行いません
 * @type switch
 * @default 0
 *
 * @param note
...
(function () {
    ...
    function readStructAchievement(parameters) {
        ...
        return {
            ...
            revealVariableId: parseIntegerParam(parameters['revealVariableId'], 0),
            revealThreshold: parseIntegerParam(parameters['revealThreshold'], 0),
            revealSwitchId: parseIntegerParam(parameters['revealSwitchId'], 0),
    ...
    function readParameter() {
        ...
        return {
            ...
            achievementMenuCols: parseIntegerParam(parameters['achievementMenuCols'], 1),
    ...
    class Window_AchievementList extends Window_Selectable {
        ...
        maxCols() {
            return Torigoya.Achievement2.parameter.achievementMenuCols;
        }
        ...
        isVisibleItem({ achievement, unlockInfo }) {
            ...
            if (!achievement.isSecret) return true;
            // タイトル画面など$gameSwitches/$gameVariablesがない場合は秘密のまま
            const hasGameData = typeof $gameSwitches !== 'undefined' && $gameSwitches &&
                                typeof $gameVariables !== 'undefined' && $gameVariables;
            if (!hasGameData) return false;
            // スイッチによる秘密解除判定
            if (achievement.revealSwitchId > 0) {
                if ($gameSwitches.value(achievement.revealSwitchId)) {
                    return true;
                }
            }
            // 変数による秘密解除判定
            if (achievement.revealVariableId > 0) {
                const variableValue = $gameVariables.value(achievement.revealVariableId);
                if (variableValue > achievement.revealThreshold) {
                    return true;
                }
            }
            return false;
