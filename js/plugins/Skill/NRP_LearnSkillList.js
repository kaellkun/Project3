 ...
 * @type boolean
 * @default false
 * @desc Display a category window to filter skills by skill set.
 * The "Note" field of each skill set is used as the category name.
 * 
 ...
 * 
 * @param UseSkillCostShowEX
 * @parent <SkillListWindow>
 * @type boolean
 * @default true
 * @desc Enable integration with NUUN_SkillCostShowEX plugin.
 * Shows MP/TP cost alongside SP cost when enabled.
 * 
 * @param <ActorWindow>
 ...
 * @type boolean
 * @default false
 * @desc スキルセット（ツリー）別のカテゴリウィンドウを表示します。
 * 各スキルセットの「メモ」がカテゴリ名として使用されます。
 * 
 * @param CategoryWindowWidth
 * @text カテゴリウィンドウ幅
 * @parent UseCategory
 ...
 * 
 * @param UseSkillCostShowEX
 * @text SkillCostShowEX連携
 * @parent <SkillListWindow>
 * @type boolean
 * @default true
 * @desc NUUN_SkillCostShowEXプラグインとの連携を有効にします。
 * 有効時、必要SP以外に使用時のMP/TPコストも表示します。
 * 
 * @param <ActorWindow>
 * @text ＜アクターウィンドウ関連＞
...
//-----------------------------------------------------------------------------
// Window_LearnSkillCategory
//
// カテゴリ選択ウィンドウクラス

function Window_LearnSkillCategory() {
    this.initialize(...arguments);
}

Window_LearnSkillCategory.prototype = Object.create(Window_Command.prototype);
Window_LearnSkillCategory.prototype.constructor = Window_LearnSkillCategory;
...
const pUseCategory = toBoolean(parameters["UseCategory"]);
const pCategoryWindowWidth = toNumber(parameters["CategoryWindowWidth"], 240);
...
const pUseSkillCostShowEX = toBoolean(parameters["UseSkillCostShowEX"], true);
// アクターウィンドウ関連
...
Scene_LearnSkillList.prototype.createSkillListWindow = function() {
    ...
    // カテゴリウィンドウが有効な場合はカテゴリウィンドウに戻る
    if (pUseCategory) {
        this._skillListWindow.setHandler('cancel', this.onSkillListCancelToCategory.bind(this));
    } else {
    ...
    // カテゴリウィンドウとの連携
    if (pUseCategory && this._categoryWindow) {
        this._categoryWindow.setSkillListWindow(this._skillListWindow);
    }
...
Scene_LearnSkillList.prototype.actorWindowRect = function() {
    ...
    // カテゴリウィンドウが有効な場合は高さを調整
    let wh;
    if (pUseCategory) {
        wh = pActorWindowHeight ? pActorWindowHeight : Math.floor(this.mainAreaHeight() * 0.6);
    } else {
        wh = pActorWindowHeight ? pActorWindowHeight : this.mainAreaHeight();
    }
    return new Rectangle(wx, wy, ww, wh);
};
 ...
 * ●カテゴリウィンドウの作成
 */
Scene_LearnSkillList.prototype.createCategoryWindow = function() {
    const rect = this.categoryWindowRect();
    this._categoryWindow = new Window_LearnSkillCategory(rect);
 ...
 * ●カテゴリウィンドウの表示領域
 */
Scene_LearnSkillList.prototype.categoryWindowRect = function() {
    const wx = 0;
    const wy = this._actorWindow.y + this._actorWindow.height;
    const ww = pCategoryWindowWidth;
    const wh = this.mainAreaHeight() - this._actorWindow.height;
    return new Rectangle(wx, wy, ww, wh);
};
 ...
 * ●カテゴリ選択決定
 */
Scene_LearnSkillList.prototype.onCategoryOk = function() {
 ...
 * ●カテゴリ選択キャンセル
 */
Scene_LearnSkillList.prototype.onCategoryCancel = function() {
    this.popScene();
};

/**
 * ●スキルリストキャンセル（カテゴリウィンドウへ戻る）
 */
Scene_LearnSkillList.prototype.onSkillListCancelToCategory = function() {
 ...
 * ●カテゴリ（スキルセット）の設定
 */
Window_LearnSkillList.prototype.setCategory = function(category) {
    if (this._category !== category) {
        this._category = category;
        this.refresh();
        // 選択はonCategoryOkで行うため、ここでは選択しない
    }
};
...
Window_LearnSkillList.prototype.refresh = function() {
    ...
    // カテゴリ機能が有効で、カテゴリが選択されている場合
    if (pUseCategory && this._category) {
        // 選択されたカテゴリ（スキルセット）のスキルのみを追加
        const skillList = parseStruct1(this._category.SkillList);
 ...
 * ※マスク表示の判定用（前提スキル等は除く）
 */
Window_LearnSkillList.prototype.isMatchSwitchCondition = function(skillData) {
    const switchNo = Number(skillData.Switch);
    // スイッチを満たしていない場合は不可
    if (skillData.Switch && !$gameSwitches.value(switchNo)) {
        return false;
...
Window_LearnSkillList.prototype.drawItemName = function(learnSkillData, x, y, width) {
    ...
    // マスク表示を行う場合は隠す（スイッチ条件のみで判定）
    if (pHiddenSkillMask && !this.isMatchSwitchCondition(learnSkillData)) {
...
Window_LearnSkillList.prototype.costWidth = function() {
    // 「必要SP: 0000」の幅 + 使用コストの幅
    const spLabelWidth = this.textWidth("必要SP: 0000");
    if (pUseSkillCostShowEX && Imported.NUUN_SkillCostShowEX && Window_SkillList.prototype.costWidth) {
        // 使用コストの幅（括弧と「使用:」含む）+ SP表示幅 + 余白
        const usageCostWidth = this.textWidth("(使用: )") + Window_SkillList.prototype.costWidth.call(this);
        return spLabelWidth + usageCostWidth + this.textWidth("  ");
    }
    return spLabelWidth;
};

/**
 * ●SP専用のコスト幅（SkillCostShowEXとの連携用）
 */
Window_LearnSkillList.prototype.spCostWidth = function() {
    return this.textWidth("必要SP: 0000");
};
...
Window_LearnSkillList.prototype.drawSkillCost = function(learnSkillData, x, y, width) {
    ...
    if (learnSkillData.SkillPoint != null) {
        // 条件を満たしていないスキルの場合は記号を表示（スイッチ条件のみで判定）
        if (!this.isMatchSwitchCondition(learnSkillData) && pHiddenSymbol) {
            this.resetTextColor();
            const symbolWidth = this.textWidth(pHiddenSymbol + " ");
            drawX -= symbolWidth;
            this.drawText(pHiddenSymbol, drawX, y, symbolWidth, "left");
        }

        // SP値を大きいフォントで描画
        this.contents.fontSize = baseFontSize + 4;
        this.changeTextColor(ColorManager.textColor(pSkillPointColor));
        const spValue = String(learnSkillData.SkillPoint);
        const spValueWidth = this.textWidth(spValue);
        drawX -= spValueWidth;
        this.drawText(spValue, drawX, y - 2, spValueWidth, "left");

        // 「必要SP: 」ラベルを描画
        this.contents.fontSize = baseFontSize;
        this.changeTextColor(ColorManager.systemColor());
        const spLabel = "必要SP: ";
        const spLabelWidth = this.textWidth(spLabel);
        drawX -= spLabelWidth;
        this.drawText(spLabel, drawX, y, spLabelWidth, "left");

        // 余白
        drawX -= this.textWidth("  ");
    }

    // === SkillCostShowEXの使用コストを描画 ===
    if (pUseSkillCostShowEX && Imported.NUUN_SkillCostShowEX && dataSkill) {
        // 閉じ括弧を描画
        this.contents.fontSize = baseFontSize;
        this.resetTextColor();
        const closeParen = ")";
        const closeParenWidth = this.textWidth(closeParen);
        drawX -= closeParenWidth;
        this.drawText(closeParen, drawX, y, closeParenWidth, "left");

        // スキルコストを描画（SkillCostShowEXの機能を使用）
        // drawSkillCostは右寄せで描画するので、drawX位置を基準にする
        if (Window_SkillList.prototype.drawSkillCost) {
            const tempWidth = drawX - x;
            Window_SkillList.prototype.drawSkillCost.call(this, dataSkill, x, y, tempWidth);
        }

        // 使用コストの幅を取得して位置を調整
        if (Window_SkillList.prototype.costWidth) {
            const skillCostWidth = Window_SkillList.prototype.costWidth.call(this);
            drawX -= skillCostWidth;
        }
        ...
        this.resetTextColor();
        const openParen = "(使用: ";
        const openParenWidth = this.textWidth(openParen);
        drawX -= openParenWidth;
        this.drawText(openParen, drawX, y, openParenWidth, "left");
    }
...
Window_LearnSkillList.prototype.updateHelp = function() {
    ...
    if (learnSkillData) {
        // ヘルプを隠す場合は追記のみ表示（スイッチ条件のみで判定）
        if (pHiddenHelp && !this.isMatchSwitchCondition(learnSkillData)) {
 ...
 * ●初期化
 */
Window_LearnSkillCategory.prototype.initialize = function(rect) {
    this._actor = null;
    this._categoryList = [];
 ...
 * ●アクター情報の設定
 */
Window_LearnSkillCategory.prototype.setActor = function(actor) {
    if (this._actor !== actor) {
        this._actor = actor;
        this.refresh();
        this.select(0);
 ...
 * ●スキルリストウィンドウの設定
 */
Window_LearnSkillCategory.prototype.setSkillListWindow = function(skillListWindow) {
    this._skillListWindow = skillListWindow;
};

/**
 * ●最大列数
 */
Window_LearnSkillCategory.prototype.maxCols = function() {
    return 1;
};
 ...
 * ●コマンドリストの作成
 */
Window_LearnSkillCategory.prototype.makeCommandList = function() {
    this._categoryList = [];
    ...
    // 条件を満たすスキルセットのみを取得
    for (const skillSet of pSkillSetList) {
        if (this.isMatchSkillSet(skillSet)) {
            // スキルセット内に表示可能なスキルがあるか確認
            const skillList = parseStruct1(skillSet.SkillList);
            let hasDisplayableSkill = false;
            for (const skillData of skillList) {
                const parseSkillData = JSON.parse(skillData);
                if (this.isDisplaySkillData(parseSkillData)) {
                    hasDisplayableSkill = true;
                    break;
                }
            }
            // 表示可能なスキルがある場合のみカテゴリに追加
            if (hasDisplayableSkill) {
                const categoryName = skillSet.Note || "その他";
                this._categoryList.push({
                    name: categoryName,
                    skillSet: skillSet
                });
                this.addCommand(categoryName, "category", true, skillSet);
            }
        }
    }
};
 ...
 * ●スキルセットの条件を満たすかどうか？
 */
Window_LearnSkillCategory.prototype.isMatchSkillSet = function(skillSet) {
    const a = this._actor;
 ...
 * ●スキルの表示条件を満たすかどうか？
 */
Window_LearnSkillCategory.prototype.isDisplaySkillData = function(skillData) {
    const skillId = Number(skillData.Skill);
    const learnedSkillDisplayStyle = skillData.LearnedSkillDisplayStyle || pLearnedSkillDisplayStyle;
 ...
 * ●スキルの表示条件を満たしているかどうか？
 */
Window_LearnSkillCategory.prototype.isMatchDisplaySkillData = function(skillData) {
    const a = this._actor;
 ...
 * ●項目の更新
 */
Window_LearnSkillCategory.prototype.update = function() {
    Window_Command.prototype.update.call(this);
    if (this._skillListWindow) {
        this._skillListWindow.setCategory(this.currentExt());
    }
};

/**
 * ●現在選択中のカテゴリ（スキルセット）を取得
 */
Window_LearnSkillCategory.prototype.currentCategory = function() {
...
//-----------------------------------------------------------------------------
// NUUN_SkillCostShowEX連携
// Window_SkillListからコスト描画用のヘルパーメソッドを継承
//-----------------------------------------------------------------------------

/**
 * ●SkillCostShowEXプラグインとの連携
 * Window_SkillListに定義されているコスト描画用メソッドをWindow_LearnSkillListでも使用可能にする
 */
if (typeof Imported !== 'undefined' && Imported.NUUN_SkillCostShowEX) {
    // コスト描画用のヘルパーメソッドを継承
    const methodsToInherit = [
        ...
        'setCostColor',
        'getEvalCost',
        'drawEquipSkillLearnSkill',
        'equipSkillLearnSkillText',
        'equipSkillLearnSkill'
    ];

    methodsToInherit.forEach(function(methodName) {
        if (Window_SkillList.prototype[methodName] && !Window_LearnSkillList.prototype[methodName]) {
            Window_LearnSkillList.prototype[methodName] = Window_SkillList.prototype[methodName];
        }
    });
}
