...
/*:zh
 * @target MZ
 * @plugindesc 提供插件参数与控制符解析的通用基础插件
 * @author triacontane
 * @url \dlc\BasicResources\plugins\official
 *
 * @help PluginCommonBase.js
 * 
 * 本插件是一个“基础插件”，用于被其他插件引用。
 * 引入本插件后，可在多种场合下使用以下控制字符。
 *
 * 【可用控制字符】
 * \v[n]  ：转换为第 n 个变量的值。  
 * \s[n]  ：转换为第 n 个开关的值（true / false）。  
 * \ss[n] ：转换为第 n 个自开关的值（true / false）。  
 * n 可以为 A, B, C, D。
 *
 * 【可使用控制字符的场合】
 * ・显示文字  
 ...
 * ・插件参数（※）  
 * ・技能、物品等说明文本  
 * ※仅对以 PluginCommonBase.js 为基础的插件有效。
 *
 * 以下内容为开发者说明：
 *
 * 当插件以本插件为基础时，可在解析参数、读取插件命令或备注栏时，
 * 自动进行参数类型识别与控制字符的转换。
 * 通过标准化控制字符解析逻辑，使所有继承此插件的插件实现一致的控制字符行为。
 *
 * 【插件参数类型解析】
 * 插件参数会被自动识别类型并转换。
 * 在读取参数时会自动处理控制字符。
 ...
 * PluginManagerEx.createParameter(document.currentScript);
 *
 * 但如果参数名称（不区分大小写）以以下结尾：
 * text / name / note / desc / script  
 * 则不会进行控制字符或类型转换，而是直接返回原始字符串。
 *
 * 【插件命令参数类型解析】
 * 插件命令参数同样会自动识别类型并转换。
 * 读取参数时也会自动解析控制字符。
 * 请使用以下方式替代原本的 PluginManager.registerCommand：
 *
 * PluginManagerEx.registerCommand(document.currentScript, "command", args => {
 * });
 *
 * 【备注栏解析】
 * 从指定对象中获取备注信息（meta），并自动转换控制字符。
 * 支持同时指定多个标签名：
 *
 * PluginManagerEx.findMetaValue(obj, tagName1, tagName2...);
 *
 * 【动态公共事件执行】
 * 可以执行动态公共事件。
 * 动态公共事件是可并行运行的堆叠型公共事件。
 * 同一个 ID 的事件可以同时多次运行，结束后自动销毁。
 *
 * 使用方式：
 * $gameMap.setupDynamicCommon(commonEventId);
 *
 * 此插件是众多 MZ 插件（如 triacontane 系列）的基础依赖，
 * 请在项目中始终启用它以确保兼容性。
 */
(() => {
    'use strict';
